...
The checklist also requires visual contract coverage for `heroSkinSprites`, `zombieCardSprites`, `skillCardSprites`, `propCoverSprites`, and `groundWashLayers` so the Cocos slice keeps pace with the current WebGL V03 reference.
