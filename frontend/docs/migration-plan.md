...
8. Map Editor Alignment：让地图编辑器产出的地标、道路、障碍、出生点符合第三版视觉和玩法标准。

## 地图编辑器策略

项目已有地图编辑器，但当前问题不是“有没有编辑器”，而是编辑器产出的地图不一定服务僵尸吃鸡体验。

地图编辑器后续需要从通用摆件工具收敛成僵尸吃鸡关卡工具：

- 地图元素分类必须清楚：可走道路、阻挡物、掩体、危险区、奖励点、出生点、尸群入口。
- 默认 palette 应优先提供第三版废土元素：破车、路障、油桶、加油站、破墙、箱子、裂路。
- 编辑器保存的地图要能直接支撑“开局尸群、升级资源、Rival 相遇、最终收圈”四件事。
- 地图预览应使用和游戏一致的 `frontend/src/theme/zombie-theme.js` 视觉标准。
